  ...
  const shotImg = async () => {
    ...
    try {
      if (editType === "grapejs" || editType === "mjml") {
        shotAction(
          // @ts-ignore
          document.getElementsByClassName("gjs-frame")?.[0]?.contentWindow
            ?.document.body
        );
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <div
          style={{ minWidth: "150px", fontWeight: "bold", fontSize: "18px" }}
        >
          在线编辑邮件
        </div>
        <div
        ...
        >
          <div>
            <InputAdornment prepend="邮件标题：">
              <Input
                value={emailTitle}
                placeholder={"请输入邮件标题"}
                onChange={(value: any) => setEmailTitle(value)}
              />
            </InputAdornment>
          </div>
          <div style={{ marginLeft: "20px" }}>
            <InputAdornment prepend="邮箱：">
              <Input
                value={emailNumber}
                placeholder={"请输入邮箱"}
                onChange={(value: any) => setEmailNumber(value)}
              />
            </InputAdornment>
          </div>
          <div>
          ...
          <div>
            <Button
              ...
              onClick={() => {
                window.open(
                  "https://github.com/yongma16/grapes-mjml-react",
                  "blank"
                );
              }}
              type="button"
      ...
      <div style={{ border: "1px solid #262626" }}>
        ...
        {editType === "color-demo" && <ColorDemo></ColorDemo>}
        {editType === "react-drag" && <ReacrDragDemo></ReacrDragDemo>}
        {editType === "monaco" && (
          <MonacoHtmlEditor ref={monacoHtmlRef}></MonacoHtmlEditor>
        )}

        {editType === "unlayer" && <UnlayerPage ref={unLayerRef}></UnlayerPage>}
        {/* {editType === "ckeditor inline" && (
          <CkeditorPage ref={ckeditorInline}></CkeditorPage>
        )} */}
        {/* {editType === "ckeditor url" && (
          <CkeditorClassic ref={ckeditorUrl}></CkeditorClassic>
        )}
        {editType === "ckeditor module" && (
          <CkeditorRender ref={ckeditorModule}></CkeditorRender>
        )} */}
      </div>
    </div>
  );
